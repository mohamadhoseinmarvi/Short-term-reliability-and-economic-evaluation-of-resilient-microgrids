...
%       A_in (n by 4 matrix with elements A_min, A_mode, A_max, A_d in the
%       respective columns 1 to 4
%       only for input vectors, not for matrices!
% returns
%       A(n by 1 by n_runs)

function [A] = MCrand2(A_in ,n_runs)

A_in(isnan(A_in)) = 0; % converting all NaN to 0
A_min = A_in(:,1);
A_mode = A_in(:,2);
A_max = A_in(:,3);
A_d = A_in(:,4);
